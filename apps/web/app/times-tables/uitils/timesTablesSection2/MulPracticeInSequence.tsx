...
const MulPracticeInSequence: React.FC<MulPracticeProps> = ({
  ...
  const renderProblem = (problem: number) => (
    <div key={problem} className="flex items-center mb-2">
      <div className="w-20 text-right pr-2">
        <span
          className={`font-mono text-lg font-bold bg-clip-text text-transparent bg-gradient-to-r ${bgGradient} whitespace-nowrap`}
        >
          {timesTable} × {problem}
        </span>
      </div>
      ...
      <div className="flex items-center">
        <span className="text-gray-500 mx-2 font-mono">=</span>
        <div className="relative">
          <input
            type="text"
            value={answers[problem] || ""}
            onChange={(e) => handleChange(problem, e.target.value)}
            className={`w-14 h-10 text-center text-lg font-mono font-semibold rounded-lg border-2 shadow-sm outline-none transition-all ${
              showResults
                ? isCorrect[problem]
          ...
          {showResults && (
            <div className="absolute -right-6 top-1/2 transform -translate-y-1/2">
              {isCorrect[problem] ? (
                ...
                <span className="text-rose-500 text-base font-medium">✗</span>
              ) : (
                <span className="text-amber-500 text-base font-medium">?</span>
              )}
            </div>
          )}
        </div>
      ...
      <div className="flex flex-col items-center w-full max-w-3xl mx-auto p-6 rounded-xl shadow-lg mt-7">
        <form onSubmit={checkAnswers}>
          <div className="grid grid-cols-2 gap-x-12 gap-y-2">
            <div className="space-y-3 border-r border-gray-200 pr-[70px]">
